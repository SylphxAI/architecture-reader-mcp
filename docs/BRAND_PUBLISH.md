# Spine — brand npm publish readiness
...
| Target brand npm id | `@sylphx/spine` |
| Brand bin | `spine` |
| Marketplace title | Spine (`server.json`) |

## Current policy

1. Ship/publish `@sylphx/architecture-reader-mcp` from `packages/mcp-server` in this repo (workspace package).
2. Optional second publish of `@sylphx/spine` **from this same repo** (same artifacts, renamed package.json at pack time).
